...
export function HookAdminPage() {
  ...
  const transferOwnershipValid = isAddress(newOwner)
    && newOwner.toLowerCase() !== ZERO_ADDRESS.toLowerCase()
    && newOwner.toLowerCase() !== owner?.toLowerCase()
  ...
  async function submitTransferOwnership() {
    if (!canWrite || !transferOwnershipValid) return;

    setTransferPending(true);
    try {
      const hash = await transferHookOwnership(newOwner as Address);
      toast.success(`Submitted ownership transfer tx: ${hash.slice(0, 10)}...`);
      await waitForTransactionReceipt(wagmiConfig, { hash });
      await refreshHookStatus();
      toast.success("Hook ownership transferred.");
      setTransferConfirmation("");
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Ownership transfer failed.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="Pool router" value={addresses.poolRouter ?? "NEXT_PUBLIC_POOL_ROUTER_ADDRESS not configured"} tone={addresses.poolRouter ? "success" : "warning"} />
      ...
      <section className="rounded-lg border border-white/10 bg-slate-900 p-5">
        <div className="flex items-start gap-3">
          <Shield className="mt-0.5 size-5 text-sky-300" />
          <div>
            <p className="text-sm text-slate-400">Ownership</p>
            <h2 className="mt-1 text-xl font-semibold text-white">Transfer hook ownership</h2>
          </div>
        </div>
        <p className="mt-3 text-sm text-slate-400">
          The new owner will control pause, pool config, emergency withdraw, and future ownership changes.
        </p>
        <div className="mt-5 grid gap-3 md:grid-cols-[1fr_220px]">
          <input
            className="h-10 rounded-md border border-white/10 bg-slate-950 px-3 text-sm text-white outline-none focus:border-sky-400"
        ...
        ) : newOwner && owner && newOwner.toLowerCase() === owner.toLowerCase() ? (
          <p className="mt-3 rounded-md border border-amber-400/20 bg-amber-400/10 p-3 text-sm text-amber-100">
        ...
        <button
          className="mt-5 inline-flex h-10 items-center justify-center rounded-md bg-sky-500 px-4 text-sm font-semibold text-slate-950 disabled:bg-slate-700 disabled:text-slate-400"
          disabled={!canWrite || !transferOwnershipValid || transferPending}
          onClick={submitTransferOwnership}
        >
          {transferPending ? "Submitting..." : "Transfer ownership"}
        </button>
      </section>
    </div>
  );
