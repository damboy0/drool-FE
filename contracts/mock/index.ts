...
export function mockMarketStats(marketId: string): MarketStats {
  ...
  return {
    totalOpenNotional: positions.reduce((sum, position) => sum + position.notional, market.totalNotional / 8n),
    activePositions: Math.max(market.openPositions, positions.length),
    averageFixedRate: market.fixedRateOffered - 0.08,
    aaveYieldDeployed: market.totalNotional / 3n,
...
export function mockOrderBook(marketId: string): SwapPosition[] {
  ...
  const seededOrders = Array.from({ length: 5 }, (_, index) => {
    ...
    return {
      ...
      floatingReceiver: MOCK_ADDRESS,
      notional,
      fixedRate: Number((market.fixedRateOffered + 0.18 - index * 0.06).toFixed(2)),
      fixedMargin: calcRequiredMargin(notional, market.fixedRateOffered, 90, market.leverageMultiplier),
      floatingMargin: 0n,
...
export async function mockOpenSwap(
  ...
  const position: SwapPosition = {
    ...
    floatingReceiver: MOCK_ADDRESS,
    notional,
    fixedRate: market.fixedRateOffered,
    fixedMargin: calcRequiredMargin(notional, market.fixedRateOffered, termDays, market.leverageMultiplier),
    floatingMargin: 0n,
...
export async function mockTakeFloatingSide(positionId: bigint, onBehalfOf: Address): Promise<Hash> {
  ...
  const matchedPosition = {
    ...
    floatingMargin: calcRequiredMargin(position.notional, market.floatingRate, termDays, market.leverageMultiplier),
    matched: true,
  };
